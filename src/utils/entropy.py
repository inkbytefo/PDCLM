...
def compute_entropy_profile_vectorized(
    ...
    # Note: unfold creates a view, so it's memory efficient
    ...
    hist.scatter_add_(1, unfolded.long(), torch.ones_like(unfolded, dtype=torch.float32))
...
def compute_entropy_profile_unoptimized(
    ...
    for i in range(0, seq_len - window_size + 1, stride):
        ...
        hist = torch.bincount(window.long(), minlength=vocab_size).float()
        hist = hist / hist.sum()
        hist = hist + 1e-10
