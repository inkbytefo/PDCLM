...
from .entropy_utils import compute_entropy_profile_vectorized
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        pse = pse.cuda()
    sample = "This is a test string for entropy pattern detection in continuous stream."
    output = pse(sample)
    print(f"Output shape: {output.shape}")
    print(f"Mean entropy modulation: {F.sigmoid(pse.scale * pse._compute_entropy_profile(torch.tensor([ord(c) for c in sample]))).mean().item():.4f}")
    
    # Performance test
    stats = pse.get_performance_stats(sample * 1000)  # 20k chars
